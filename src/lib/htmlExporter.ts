export function exportToHTML(components: any[], title: string): string {
  const css = `
  ...
  const body = components.map(c => renderComponent(c)).join('\n');
...
function renderComponent(c: any): string {
  ...
  switch (c.type) {
    case 'navbar':
      return `<nav style="background:${p.bgColor};color:${p.textColor};padding:16px 24px"><div class="container flex justify-between items-center"><strong style="font-size:20px">${p.brand}</strong><div class="flex gap-6">${(p.links||[]).map((l:string) => `<a href="#" style="color:${p.textColor};text-decoration:none">${l}</a>`).join('')}</div></div></nav>`;
    case 'hero':
      return `<section class="section" style="background:${p.bgColor};color:${p.textColor};text-align:center;padding:120px 24px"><div class="container"><h1 style="font-size:56px;font-weight:800;margin-bottom:24px">${p.heading}</h1><p style="font-size:20px;opacity:0.8;max-width:600px;margin:0 auto 40px">${p.subheading}</p>${p.buttonText ? `<a href="#" style="display:inline-block;background:${p.buttonColor||p.accentColor};color:#fff;padding:16px 40px;border-radius:12px;text-decoration:none;font-weight:600;font-size:18px">${p.buttonText}</a>` : ''}</div></section>`;
    case 'features':
